...
| **深度** | D4 专深 |
| **锚点** | **T1**（`echo UDC`） |
| **前置** | [`02-architecture-overview.md`](02-architecture-overview.md) §3–§5 |
| **本文** | `udc/core.c`：配对、bind 链、pending、pullup |
| **下一步** | [`07-composite-ep0-enumeration.md`](07-composite-ep0-enumeration.md) |

> 硬件寄存器见 [`A3-dwc2-soft-connect.md`](A3-dwc2-soft-connect.md)；configfs 组装见 [`03-configfs-assembly.md`](03-configfs-assembly.md) §5。
...
`dwc2_gadget_init()` 末尾调用 `usb_add_gadget_udc(dev, &hsotg->gadget)`（详见 [`A1-dwc2-board-probe.md`](A1-dwc2-board-probe.md) §5）。
...
| 3 | `dev_set_name(&udc->dev, "%s", kobject_name(&parent->kobj))` → **`49000000.usb-otg`** |
...
configfs 总是设置 `udc_name`（如 `49000000.usb-otg`），故走 **按名匹配** 路径。
...
| 步骤 | 函数 | L3 语义 | 下层（附录） |
|------|------|---------|--------------|
| ① | `gadget_driver->bind` | 装配 `cdev`、字符串、function | `configfs_composite_bind` → [`03`](03-configfs-assembly.md) |
| ② | `usb_gadget_udc_start` | 启动 UDC 硬件侧 gadget 模式 | dwc2 `dwc2_hsotg_udc_start` → [`A2`](A2-dwc2-pg71-init.md) |
| ③ | `usb_udc_connect_control` | 若 `udc->vbus` 则 pullup | `usb_gadget_connect` → [`A3`](A3-dwc2-soft-connect.md) |

**注意**：`composite->bind`（`usb_composite_driver` 层）在 configfs 下为 **`configfs_do_nothing`**，不被调用；组装全在 ① 完成。
...
| **`soft_connect` sysfs** | `echo connect > .../soft_connect` | L3 直接 `usb_gadget_connect` | 调试；见 [`A3`](A3-dwc2-soft-connect.md) |
| **role-switch** | fusb302 UFP | `drd.c` → `dwc2_hsotg_core_connect` | Type-C 会话；见 [`A1`](A1-dwc2-board-probe.md) §7 |
...
**框架语义**：L3 只认 `usb_gadget_connect/disconnect`；寄存器细节在 L5。
...
| 回调 | configfs 实现 | 何时 |
|------|---------------|------|
| `setup` | `configfs_composite_setup` → `composite_setup` | Host EP0；见 [`07`](07-composite-ep0-enumeration.md) |
...
dwc2 EP0 中断最终调 **`hsotg->driver->setup()`**（[`A4`](A4-dwc2-ep0-control.md)）。
...
| **T2** | 不在 L3；Host `SET_CONFIGURATION` 在 L2 composite（[`07`](07-composite-ep0-enumeration.md)） |

T1 之后 Host **可以** 开始枚举（读描述符）；T2 之后 ACM **数据面** 才通（[`08`](08-function-acm-path.md)）。
...
- 架构：[`02-architecture-overview.md`](02-architecture-overview.md) §4–§5
- configfs bind 内容：[`03-configfs-assembly.md`](03-configfs-assembly.md) §5、[`04-create-kernel-map.md`](04-create-kernel-map.md) §2.16
- 回调速查：[`05-kernel-reference.md`](05-kernel-reference.md) §3–§4
- 板级 UDC 注册：[`A1-dwc2-board-probe.md`](A1-dwc2-board-probe.md) §5
