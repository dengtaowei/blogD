# 07 · Composite EP0 与枚举

| | |
|---|---|
| **层** | L2 Composite |
| **深度** | D4 专深 |
| **锚点** | **T1→T2**（枚举 → `SET_CONFIGURATION`） |
| **前置** | [`06-udc-core-bind.md`](06-udc-core-bind.md)（bind 后 `setup` 已挂好） |
| **本文** | `composite.c`：`composite_setup`、`set_config` |
| **下一步** | [`08-function-acm-path.md`](08-function-acm-path.md) |

> dwc2 EP0 硬件/DMA 见 [`A4-dwc2-ep0-control.md`](A4-dwc2-ep0-control.md)；configfs 描述符组装见 [`03-configfs-assembly.md`](03-configfs-assembly.md)。
...
**关键**：ACM 的 bulk IN/OUT 在 **`acm_set_alt`（data interface）** 里 `usb_ep_enable` + `gserial_connect`（见 [`08`](08-function-acm-path.md)）。
...
- 仍走 EP0，与 [`08`](08-function-acm-path.md) 数据面分离
...
[`04-create-kernel-map.md`](04-create-kernel-map.md) 按时间线对照 create 脚本逐步；本文按 **EP0 请求类型** 对照。
...
lsusb 样例见 [`01-acm-lsusb-dump.txt`](01-acm-lsusb-dump.txt)。
...
- UDC bind / pullup：[`06-udc-core-bind.md`](06-udc-core-bind.md)
- ACM `set_alt` / 数据面：[`08-function-acm-path.md`](08-function-acm-path.md)
- EP0 硬件：[`A4-dwc2-ep0-control.md`](A4-dwc2-ep0-control.md)
- 速查表：[`05-kernel-reference.md`](05-kernel-reference.md) §2
