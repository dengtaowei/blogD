...
homeDesc: V-blank、TEM=0/1、抓沿选型与写速约束
sidebarOrder: 10
...
> **本文**：TE 是什么、屏侧如何打开、`TEM=0/1` 波形差异、主机抓沿与写屏速度的关系
...
屏按自己的节奏扫描 GRAM 并点亮像素。主机若在扫描途中随意改显存，同一帧画面可能上下两截分属新旧内容，即撕裂。TE 标出相对安全的相位（典型是垂直消隐），主机再开始写下一帧。

可以记成：**TE = 屏发出的「可以刷了」节拍，不是主机写出来的。**
...
帧周期取同名沿间隔（上升→上升或下降→下降）；上表用高、低电平相加，与另一次测得的下降沿间隔约 **16.831 ms**（≈ 59.4 Hz）一致。手册「60 Hz」是标称档位：内部 OSC、porch、总行数都会让周期在附近漂移，并非精确锁定的 60.000 Hz。
...
因此一帧内不再是「一个宽脉冲」，而是 **大量窄脉冲**（数量接近屏高）。

### 4.1 实测波形

![TEM=1 时 TE 波形：约一行一脉冲](/files/tem_1.png)

同一次测量里，光标跨度约 **16.563 ms**（仍约一帧），其间上升沿约 **321** 次——与 320 行量级一致。时间轴较宽时，窄脉冲会糊成「实心白块」。
...
主机用 GPIO 边沿中断对齐 TE。`TEM=0` 时 TE 在 V-blank 为高，因此：

| 沿 | 时刻 | 常见用法 |
|----|------|----------|
| 上升沿 | V-blank **开始** | 主机写得**快**时：尽量在消隐内起笔，让本帧扫描读到新数据 |
| 下降沿 | V-blank **结束** / 有效区开扫 | 主机写得**慢**时：写指针落在扫描线后面，本帧继续显示旧内容，新内容留给下一帧 |

业界常见结论（SSD1963 应用笔记、ST7789 手册「MPU 快/慢于 panel read」两例、乐鑫 TE 防撕裂说明等）并不是「一律上升沿」：

1. **先开 TE**（`0x35`，一般 `TEM=0`），需要时再用 `0x44` 调 tear scanline。  
2. **用哪条沿取决于写速 vs 扫描速**：快用上升沿抢消隐；慢用下降沿「跟在扫描后面写」。  
3. TE 是**相位标记**，不是「高电平期间才能写」的窗口；写完一帧仍须满足带宽（乐鑫侧还常要求写速不低于扫描速的约一半，否则即使用 TE 仍可能中途交叉）。  
4. 最终以运动画面实测为准；带宽不够时优先提总线时序、缩小更新区域或降低换帧率。
...
「2× 周期」是工程折中：单帧传不完一场的安全窗时，用更稀的对齐节奏，降低写指针与扫描线交叉的概率；它**不能**替代足够的写带宽。TE 只决定**从哪个相位起笔**；若整帧 DMA / 总线时间仍长于一场扫描，扫描线仍可能追上写入区域，画面仍可能撕裂。
...
也可每 N 个 TE 才写一帧：屏 TE ≈ 60 Hz、N=3 时，换图大约 20 次/秒。TE 决定**相位与节拍**；单帧能否在下一拍前送完，仍看写屏带宽。
...
1. **TE** 由屏在扫描节拍下产生，用于主机对齐写 GRAM，减轻撕裂。  
2. **`TEON` + `TEM=0`**：每帧一个宽脉冲，高电平 ≈ V-blank（实测约 1.16 ms）；周期在标称 60 Hz 附近波动属正常。  
3. **`TEM=1`**：叠加行消隐窄脉冲（实测约 321 次/帧）；整帧刷屏路径一般不要开。  
4. **抓沿**：按写速相对扫描速选择上升沿或下降沿；快路径常见 1× + 上升沿，慢路径常见 2× + 下降沿。  
5. **工程上**：TE 管相位；传完慢于扫描时，即使用 TE 仍可能撕裂。
...
- [Espressif：LCD Screen Tearing](https://docs.espressif.com/projects/esp-iot-solution/en/latest/display/lcd/lcd_screen_tearing.html) — TE 同步与写/读速度关系  
