...
> **本文**：板上 `/dev/snd` 下面的节点含义、创建路径、以及 `open` 这些 PCM 节点时如何进到 `soc_pcm_open`
...
> **板上 `/dev/snd` 里每个节点对应什么功能？和 Machine 的三条 `dai_link`、打开时的 `file_operations` 如何对应？`open(pcm…)` 之后 ASoC 做了什么？**
...
声卡名来自设备树 `model = "wm8960-audio"`。device 1 为 ASRC 前端，说明见 [§3.1](#31-asrcdevice-1)。

---

## 3. 节点与三条 `dai_link`

Machine 驱动 `sound/soc/fsl/imx-wm8960.c` 里注册三条 `dai_link`，再加一个控制节点：
...
| 控制节点 | — | `controlC0` | mixer（音量、通路等）；`amixer` / `tinymix` 等控制使用 |
...
本板 DTS 挂了 `asrc-controller` 时，Machine 会多建两条 `dai_link`：前端 FE 对应 `pcmC0D1*`，后端 BE 标 `no_pcm`，**不会**出现独立 `/dev` 节点。本文对这条路径不做赘述。
      ...
      → snd_card_register
          → snd_device_register_all
              ...
              → pcm:     snd_pcm_dev_register
                  → snd_register_device(...)
                      → device_add → 这时用户态才可看见 /dev/snd/xxx
```
...
| 建声卡 | `sound/core/init.c` — `snd_card_new()` / `snd_card_register()` |

`snd_card_new()` 里会调用 `snd_ctl_create()` **预备** `controlC0`，但用户态真正可访问要等 `snd_card_register()`。
...
control 节点注册（`sound/core/control.c`）：
...
§4.2 / §5 说到首次 `open` 会换成 `snd_pcm_playback_open` / `snd_pcm_capture_open`。对 **HiFi（`pcmC0D0p` / `pcmC0D0c`）** 来说，真正接到 ASoC 各层的是 `substream->ops->open` → **`soc_pcm_open`**（`sound/soc/soc-pcm.c`）。建 PCM 时在 `soc_new_pcm` 里写入：`rtd->ops.open = soc_pcm_open`。
...
soc_pcm_open(substream)
  ...
  ├─ 3. 非 DPCM：soc_pcm_init_runtime_hw（CPU DAI/Codec 能力求交 → runtime->hw）
...
此阶段 **一般不** 配具体采样率、不开环形 DMA 传数据；那些在后续 `hw_params` / `prepare` / `trigger`（见播/录路径文）。
...
- 打开 `pcmC0D0p` / `pcmC0D0c` 时，经 `snd_pcm_open` 进入 **`soc_pcm_open`**：依次调用 SAI / Platform / Machine 的 `startup`（或 `open`），再把 CPU DAI 与 Codec 能力求交写入 `runtime->hw`。
